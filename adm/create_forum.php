...
$next_url="../viewtopic.php?f=".$forum_check['forum_id'];
...
if(isset($node_id) && isset($node_desc)){
	    ...
	    echo "redirect to general discussion thread";
	    $tid_a=$db->sql_query("select topic_id from phpbb_topics where forum_id=$forum_check[forum_id] AND topic_title='general discussion'");
		$tid=$db->sql_fetchrow($tid_a);
	    $next_url=$next_url."&t=".$tid['topic_id']."&sid=".$sid_f;
		echo $next_url;
	    header("Location: $next_url");
