...
// Variables added for Dragoon forum - author, section, problemname

$problem = request_var('problem', '');
$author = request_var('author', '');
$section = request_var('section', '');
...
// **********************
// Dragoon forum handling
// If no topic ID passed in and author/section/problem passed in instead, determine topic id or create new topic
...
if(!$topic_id && $problem != ''){
	error_log("problem name detected, beginning process to post/open thread",0);
		...
		$topic = (int) $db->sql_fetchfield('topic_id');
		$db->sql_freeresult($result);
		
		// Add user to group if they're not there already
			error_log('Checking if user is in a group',0);
			$user_id = $user->data['user_id'];
			$username= $user->data['username'];
			$sql = "SELECT group_id FROM phpbb_groups 
					WHERE group_name = '$section'";
			$result = $db->sql_query($sql);
			$group_id = (int) $db->sql_fetchfield('group_id');
			error_log("group_id is $group_id");
			$db->sql_freeresult($result);
			$sql = "SELECT group_id FROM phpbb_users
					WHERE user_id = '$user_id'";
			$result = $db->sql_query($sql);
			$user_group = (int) $db->sql_fetchfield('group_id');
			error_log("user_group is $user_group");
			$db->sql_freeresult($result);
			if(!($user_group == $group_id) && ($user_group < 4 || $user_group > 6)){
		...
		if(!$topic){
			error_log('No topic found, beginning make topic work',0);
			// Get forum id for this section
			$sql = "SELECT forum_id FROM " . FORUMS_TABLE . "
					WHERE forum_name = '$section'";
			$result = $db->sql_query($sql);
			...
			$mode = 'post';
			$subject = "Discussion of $problem by $author for class $section";
			$topic_type = POST_NORMAL;
			$poll = "";
			
			$text = utf8_normalize_nfc("This is the discussion thread for problem $problem");
